...
const Project = () => {
    ...
    // WriteAiMessage now expects an object with a 'text' key.
    function WriteAiMessage(messageObj) {
    ...
    useEffect(() => {
        ...
        receiveMessage('project-message', data => {
            ...
            if (data.sender._id === 'ai') {
                // Check if data.message is a string; if so, parse it. Otherwise, assume it's already an object.
                let aiMessage;
                if (typeof data.message === 'string') {
                    try {
                        aiMessage = JSON.parse(data.message)
                    } catch (error) {
                        console.error("Error parsing AI message string:", error)
                        return
                    }
                } else {
                ...
                // Save message with parsed message content
                setMessages(prevMessages => [...prevMessages, { sender: data.sender, message: aiMessage }])
            } else {
                        ...
                        <div className="files flex overflow-x-auto scrollbar-hide">
                            {
                                openFiles.map((file, index) => (
                                    <div
                                        key={index}
                                        className={`open-file group flex items-center w-fit gap-2 ${currentFile === file ? 'bg-slate-400' : 'bg-slate-300'}`}>
                                        <button
                                        ...
                                        <button
                                            ...
                                            className="p-2 opacity-0 group-hover:opacity-100 hover:bg-slate-500 rounded-sm">
